...
function moveSantaWithCurvedPath() {
  const santaContainer = document.querySelector('.santa-container');
  let positionX = -10; // Bắt đầu từ ngoài màn hình bên trái
  const speed = 2; // Tốc độ di chuyển
  const centerY = window.innerHeight * 0.3; // Trung tâm quỹ đạo (50% chiều cao màn hình)
  const radius = window.innerHeight * 0.2; // Bán kính quỹ đạo (20% chiều cao màn hình)
  ...
  function createTrail(x, y) {
    ...
    // Xóa vệt sáng sau khi di chuyển hết
    setTimeout(() => {
      trail.remove();
    }, 1000); // Vệt sáng tồn tại 1 giây
  }

  // Hàm di chuyển ông già Noel theo quỹ đạo vòng cung
  function move() {
    // Tính toán quỹ đạo vòng cung
    const angle = positionX * Math.PI / window.innerWidth; // Góc của vòng cung
    const offsetY = Math.sin(angle) * radius; // Tính toán vị trí theo chiều dọc

    santaContainer.style.left = `${positionX}px`; // Di chuyển ông già Noel theo chiều ngang
    santaContainer.style.top = `${centerY - offsetY}px`; // Di chuyển theo chiều dọc

    // Tạo vệt sáng từ vị trí cáng xe
    const santaRect = santaContainer.getBoundingClientRect();
    const leftCangX = santaRect.left + santaRect.width * 0.2; // Vị trí cáng trái
    const rightCangX = santaRect.right - santaRect.width * 0.2; // Vị trí cáng phải
    const santaY = santaRect.top + santaRect.height * 0.7; // Phát vệt sáng từ phía dưới

    // Tạo vệt sáng từ cáng trái và phải
    createTrail(leftCangX, santaY);
    

    // Di chuyển ông già Noel từ trái qua phải
    positionX += speed;

    // Reset vị trí khi ra khỏi màn hình
    if (positionX > window.innerWidth) {
      positionX = -10; // Quay lại vị trí bắt đầu
    }

    requestAnimationFrame(move);
  }

  move(); // Bắt đầu di chuyển
}

// Gọi hàm di chuyển ông già Noel
moveSantaWithCurvedPath();
...
/*
  ...
  */
  ...
  for (let i = 0; i < lights; i++) {
    ...
    light.style.top = `${Math.random() * 100}%`;
    light.style.animationDelay = `${Math.random() * 2}s`;
    light.style.background = `hsl(${Math.random() * 360}, 100%, 770%)`;
    tree.appendChild(light);
  }
}
